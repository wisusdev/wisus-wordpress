...
function wisus_config_theme($wp_customize)
{
	...
	// Add container fluid option
	$wp_customize->add_setting('wisus_container_fluid', array(
		'default' => false,
		'sanitize_callback' => 'sanitize_text_field',
		'transport' => 'refresh',
	));

    // Add control for container fluid
    $wp_customize->add_control('wisus_container_fluid_control', array(
        'label' => __('Container Fluid', 'wisus'),
        'section' => 'wisus_layout_options',
        'settings' => 'wisus_container_fluid',
        'type' => 'checkbox',
    ));
}
